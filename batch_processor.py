...
def extract_text_safely(html_content):
    ...
    for mjx in el.find_all(class_=lambda c: c and 'mjx-chtml' in c): mjx.decompose()
...
async def fetch_question(context, url: str):
    page = await context.new_page()
    try:
        # wait for 15secs max before giving up on the URL
    ...
    if category_element: question_data["subject"] = category_element.text.strip()
...
async def process_single_url(url, context, outfile, semaphore, write_lock, progress):
    async with semaphore:
        try:
            extracted_data = await fetch_question(context, url)
            async with write_lock:
                ...
                progress["count"] += 1
                print(f"[{progress['count']}/{progress['total']}] Successfully extracted: {url}")
                
        except Exception as e:
            # log in case of failure
            async with write_lock:
                progress["count"] += 1
                print(f"[{progress['count']}/{progress['total']}] [!] ERROR on {url}: {e}")
        
        # throttle to avoid server ddos
        await asyncio.sleep(0.5)


async def process_urls(csv_filepath, output_jsonl):
    urls_to_process = []
    ...
    try:
        with open(csv_filepath, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                if len(row) >= 2:
                    urls_to_process.append(row[1]) 
    except FileNotFoundError:
        print(f"Error: Could not find {csv_filepath}.")
        return

    # build processed URL set
    processed_urls = set()
        ...
        with open(output_jsonl, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    ...
                    processed_urls.add(data.get("source_url"))
                except json.JSONDecodeError:
                    pass
        print(f"Resuming job: {len(processed_urls)} URLs already processed.")

    # filter out processed URLs 
    pending_urls = [url for url in urls_to_process if url not in processed_urls]
    ...
    print(f"Found {total_pending} pending URLs to process.")

    MAX_CONCURRENT_TABS = 5
    semaphore = asyncio.Semaphore(MAX_CONCURRENT_TABS)
    write_lock = asyncio.Lock()
    progress = {"count": 0, "total": total_pending}

    # run the task queue with 1 browser instance
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        context = await browser.new_context(java_script_enabled=False)
        with open(output_jsonl, 'a', encoding='utf-8') as outfile:
            tasks = [
                process_single_url(url, context, outfile, semaphore, write_lock, progress)
        ...
        await browser.close()
        print("\nBatch extraction complete! All data saved to:", output_jsonl)

if __name__ == "__main__":
    INPUT_CSV = "./data/go_question_urls.csv" 
    OUTPUT_JSONL = "./data/questions_dataset.jsonl"
    
    # run orchestration
    asyncio.run(process_urls(INPUT_CSV, OUTPUT_JSONL))